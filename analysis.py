...
def main():
    ...
    friends_base = ['/local/scratch/hdd/gallim/friends/' + f for f in ['SVFit/', 'MELA/', 'FakeFactors/']]
    eras = ['2017']
    channels = ['mt']
    units = {era: {channel: {} for channel in channels} for era in eras}

    units['2017']['mt']['data'] = Unit(
            dataset_from_artusoutput('data', cfg17.datasets.files['singlemuon'], 'mt_nominal', ntuples_base, friends_base),
            [cfg17.channels.mt], hists)

    dy_dataset = dataset_from_artusoutput('dy', cfg17.datasets.files['DY'], 'mt_nominal', ntuples_base, friends_base)
    ...
    tt_dataset = dataset_from_artusoutput('tt', cfg17.datasets.files['TT'], 'mt_nominal', ntuples_base, friends_base)
    ...
    vv_dataset = dataset_from_artusoutput('vv', cfg17.datasets.files['VV'], 'mt_nominal', ntuples_base, friends_base)
    ...
    units['2017']['mt']['w'] = Unit(dataset_from_artusoutput('w', cfg17.datasets.files['W'], 'mt_nominal', ntuples_base, friends_base),
            [cfg17.channels.mt, cfg17.processes.w('mt')], hists)

    units['2017']['mt']['ggh'] = Unit(
            dataset_from_artusoutput('ggh', cfg17.datasets.files['ggH'], 'mt_nominal', ntuples_base, friends_base),
            [cfg17.channels.mt, cfg17.processes.ggh('mt')], hists)
    units['2017']['mt']['qqh'] = Unit(
            dataset_from_artusoutput('qqh', cfg17.datasets.files['qqH'], 'mt_nominal', ntuples_base, friends_base),
    ...
    r_manager.run_locally('output.root', nworkers=12, nthreads=1)
